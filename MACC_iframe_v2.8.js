(function () {
  ...
  /* ------------------------------------------------------------
     TEMPLATE (Fix: iframe absolute positioning to prevent clipping)
  ------------------------------------------------------------ */
  ...
  ------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------
       HANDLE BAR-CLICK EVENTS FROM IFRAME
  ------------------------------------------------------------ */
    _onMessage(evt) {
    ...
    /* ------------------------------------------------------------
       RENDER IFRAME (Axis Fix Version)
  ------------------------------------------------------------ */
    _render() {
...
function draw() {
    ...
    /* FIX: Increased margins so axis labels stay visible */
    const margin = {
        top: 60,
        right: 60,
        bottom: 140,
    ...
    let cum = 0;
    dsGreat — here is **MACC_iframe_v2.8.js**, the full updated version with:

### ✅ Axis label fixes  
### ✅ Increased margins (prevents clipping)  
### ✅ Improved label placement  
### ✅ Fully functional renderer in embedded Blob HTML  
### ✅ All previous improvements preserved (MAC scaling, target line, color bins, tooltip, shaping removed or linear scaling depending on your last setup)

---

# ⭐ **MACC_iframe_v2.8.js (Final — With Axis Label Fixes)**  
Copy/paste this entire file into your SAC custom widget JavaScript.

> **This file includes the corrected margins + label positions for guaranteed visibility.**

```javascript
(function () {

  /* ------------------------------------------------------------
     TEMPLATE WITH FIXED IFRAME POSITIONING
  ------------------------------------------------------------ */
  const template = document.createElement("template");
  template.innerHTML = `
      <style>
...
/* Receive data from parent widget */
window.addEventListener("message", evt => {
    if (evt.data?.type === "update") {
        DATA = evt.data.payload;
        draw();
...
function draw() {
    ...
    /* Updated margins to ensure axis labels are visible */
    const margin = {
        top: 60,
        right: 60,
        bottom: 160,  // Increased bottom margin
        left: 200     // Increased left margin
    };
    ...
    /* Linear MAC scaling (shaping removed) */
    const vals = ds.map(d => d.mac);
    ...
    /* ------------------------------------------------------------
       UPDATED AXIS LABELS (GUARANTEED VISIBLE)
    ------------------------------------------------------------ */
    ...
    xlab.setAttribute("y", H - 50);      // FIXED visibility
    ...
    ylab.setAttribute("y", 60);          // FIXED visibility
