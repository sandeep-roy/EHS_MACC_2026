(function () {
    ...
    class VariableWidthMACC extends HTMLElement {
        ...
        /* SAFE INGEST FOR YOUR SAC STRUCTURE */
        _ingest(binding) {
...
function draw() {
    ...
    // ----------------------------
    // MAC SHAPING (Exponent = 0.7)
    // ----------------------------
    const SHAPE = 0.7;
    ds.forEach(d => {
        d.macShaped = Math.sign(d.mac) * Math.pow(Math.abs(d.mac), SHAPE);
    });

    // ----------------------------
    // Y SCALE WITH SHAPING + PADDING
    // ----------------------------
    ...
    for (let i=0; i<=ytCount; i++){
        const t = minMAC + (i/ytCount)*(maxMAC-minMAC);
        const yy = y(t);
        ...
        lbl.textContent = Math.round(
            Math.sign(t) * Math.pow(Math.abs(t), 1/SHAPE)
        );
