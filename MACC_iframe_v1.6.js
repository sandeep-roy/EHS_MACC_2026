(function () {
    ...
    class VariableWidthMACC extends HTMLElement {
        ...
        /* SAFE INGEST (Your actual SAC structure) */
        _ingest(binding){
            ...
            const html = `
                ...
                <script>
                    ...
                    function draw() {
                        ...
                        /* ---------------------------
                           SAFE Y SCALE with BOUNDS
                           --------------------------- */
                        let macVals = ds.map(d=>d.mac).filter(v=>isFinite(v));
                        let maxMAC = Math.max(...macVals);
                        let minMAC = Math.min(...macVals);

                        // Clamp extreme MAC values
                        const RANGE_MIN = -1500;
                        const RANGE_MAX = 2000;
                        maxMAC = Math.min(maxMAC, RANGE_MAX);
                        minMAC = Math.max(minMAC, RANGE_MIN);
                        ...
                        ds.forEach(d=>{
                            ...
                            const bw = Math.max(4, x(d.abate)-x(0));   // min width
